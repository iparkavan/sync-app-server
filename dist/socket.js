...
const SetupSocket = (server) => {
    const io = new socket_io_1.Server(server, {
        cors: {
            origin: [app_config_1.config.FRONTEND_ORIGIN, app_config_1.config.LOCAL_ORIGIN],
    ...
    const sendMessage = async (message) => {
        try {
            const createdMessage = await messages_model_1.default.create(message);
            const messageData = await messages_model_1.default.findById(createdMessage._id)
    ...
    const sendChannelMessage = async (message) => {
        try {
            ...
            const createdMessage = await messages_model_1.default.create({
                sender,
                recipient: null,
                content,
                messageType,
            ...
            if (!messageData)
                return;
            await chennel_model_1.default.findByIdAndUpdate(channelId, {
                $push: { messages: createdMessage._id },
            });
            const channel = await chennel_model_1.default.findById(channelId).populate("members");
            if (channel && channel.members) {
                ...
                if (channel.admin) {
                    const adminSockets = userSocketMap.get(channel.admin._id.toString()) || [];
    ...
    io.on("connection", (socket) => {
        ...
        socket.on("typing", async ({ recipientId }) => {
            ...
            const sender = await user_model_1.default.findById(userId).select("firstName lastName profileImage");
            if (sender) {
